...
| Type           | Purpose |
|----------------|---------|
| `Session`      | Root aggregate: `original_objective`, `project_root`, `root_task_id`, `status`, timestamps. Carries the **Phase E** session budget: config `max_task_runs`, `max_wall_seconds`, `headless` (explore defaults to unlimited/off; a greenfield session seeds a finite `GREENFIELD_MAX_TASK_RUNS=60` / `GREENFIELD_MAX_WALL_SECONDS=1800` backstop in `SessionRunner.start_session` unless the caller passes an explicit cap) plus live counters `task_runs` and `first_task_started_at`. The store round-trips the new fields with backward-compatible `.get()` defaults, so pre-existing sessions load unchanged. |
...
* `on_task_completed(session, completed, tasks)` -- dispatch via the
  ...
  APPLY (greenfield,
         failed_files) -> SCAFFOLD  (Fix G1: an APPLY that parses-and-drops
                                     ...
                                     package.json, per
                                     _dropped_foundational_files) skips the
                                     per-file regenerate budget and
                                     escalates straight to a re-plan
                                     (_replan_or_fail, REPLAN_BUDGET=1))
