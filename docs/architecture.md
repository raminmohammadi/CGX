...
**Scaffold output budget** (`get_scaffold_budget`). Whole-file scaffold
generation reuses the same prompt-context caps as `get_summary_budget` but
raises `output_tokens` to a tier-indexed whole-file ceiling
(`_SCAFFOLD_OUTPUT_TOKENS`: 4K / 8K / 12K / 16K by tier). Too tight a cap
truncates the body mid-file, the partial JSON fails to parse, and APPLY's
syntax gate silently drops the file; as a backstop
`cgx.answer.engine._blocking_scaffold_call` detects a length-capped stop
...
* `on_task_completed(session, completed, tasks)` -- dispatch via the
  ...
  APPLY (greenfield,
         failed_files) -> SCAFFOLD  (Fix G1: an APPLY that parses-and-drops
                                     ...
                                     FAILED; but a dropped *foundational*
                                     manifest (requirements*.txt /
                                     pyproject.toml / setup.{py,cfg} /
                                     package.json) that failed a
                                     *structural* gate (per
                                     _structural_foundational_failures)
                                     skips the per-file regenerate budget
                                     and escalates straight to a re-plan
                                     (_replan_or_fail, REPLAN_BUDGET=1); a
                                     manifest dropped only for a bare empty
                                     patch is excluded -- a transient miss
                                     the generator recovers in place (path-
                                     less freeform fence + one empty-body
                                     retry), so it takes the per-file
                                     regenerate instead)
  ...
  REPAIR (strategy=regenerate, Phase 6.1)
                       -> SCAFFOLD  (router walks up to nearest
                                     ...
                                     prior_failure_signatures are folded
                                     into the fresh SCAFFOLD and threaded
                                     down its APPLY -> ... -> VERIFY chain
                                     so an identical failure after the
                                     regenerate is stopped by the flap
                                     detector; syntax churn capped at
  ...
  REPAIR (strategy=install_deps)
                       -> BOOTSTRAP_ENV (missing_dependency verdict:
                                     ...
                                     absent package(s) + sync
                                     requirements.txt, then re-probe via
                                     API_CHECK -> SMOKE -> VERIFY; the
                                     shared repair budget + flap guard
...
* `on_budget_exhausted(session, over_task, tasks, reason)` --
  session-level circuit breaker (**Phase E**). Greenfield sessions get
  finite `GREENFIELD_MAX_TASK_RUNS` / `GREENFIELD_MAX_WALL_SECONDS`
  defaults (see the data-model note above) so an autonomous build always
  halts even if it slips past the per-loop caps. When the runner detects
...
1. **Contract-first `DECOMPOSE`.** The planner emits a `WORK_PLAN`
   ...
   **Mandatory cross-seam endpoints (P0a).** A manifest is only as safe as
   the contract it declares, and the planner routinely omits the one that
   matters most: the HTTP shape a JS frontend and a Python backend must
   share. `_is_client_server_manifest` flags a manifest that pairs a
   `.jsx`/`.tsx`/`.vue` client with a Python route (a canonical entry
   basename like `app.py`/`main.py`, a Flask/FastAPI/Django/Express skill
   or goal keyword, or a route signal in a file description). For such a
   seam with no `endpoints` contract, `_ensure_cross_seam_endpoints` runs
   one bounded, temperature-0 `extract_endpoint_contracts`
   (`cgx.answer.engine`) pass over the goal + manifest to recover it; if
   that still produces nothing the executor returns a **non-`retryable`
   (terminal)** failure rather than scaffold the seam contract-free -- so
   the P0b payload gate below always has a contract to enforce and a
   request-key rename can never reach `VERIFY` uncaught. Pure-frontend and
