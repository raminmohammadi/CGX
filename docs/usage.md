...
Averix supports four provider kinds, all configurable from the **⚙️ Setup**
...
`<project_root>/.averix-backups/<run_id>/` for any pre-existing file
it overwrites). The `verify` task runs the generated tests.

**Skills the system knows about.** Averix ships with a registry of
...
[`extension/`](../extension/) hosts the Averix web UI inside a VS Code
...
`<project_root>/.averix-backups/<run_id>/` and returns the path on
...
curl -X POST http://localhost:8765/api/rollback \
  ...
  -d '{"project_root": "/path/to/proj",
       "backup_dir": "/path/to/proj/.averix-backups/<run_id>"}'
```
