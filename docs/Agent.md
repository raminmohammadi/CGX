...
* `on_task_completed(session, completed, tasks)` -- an executor
  ...
  - Greenfield loop: `CLARIFY_REQUIREMENTS → ASK_USER(clarify_answers)`,
    ...
    `package.json`, detected by `_dropped_foundational_files`) skips
    the per-file regenerate budget entirely -- re-asking the same weak
    model reproduces the drop -- and escalates straight to a re-plan
    (`_replan_or_fail`, `REPLAN_BUDGET=1`) that restructures the plan
    so each manifest is re-emitted as a minimal valid file; the same
    guard fronts the SCAFFOLD-failed edge),
