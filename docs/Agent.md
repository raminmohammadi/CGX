...
| `Fact`         | An append-only piece of session knowledge (`FILE`, `SYMBOL`, `PARAMETER`, `ANCHOR`, `LLM_CALL`, or `REPAIR_LEDGER`). `LLM_CALL` facts (**Phase 5.1**) carry `{provider, model, sampling_params, prompt, response, latency_ms, tokens_in, tokens_out, source_task_id, role}` recorded by every LLM call site in `cgx.answer.engine`, `clarify_requirements.py`, `decompose.py`, and `scaffold.py`. `REPAIR_LEDGER` facts (**Phase 2**) hold the durable working memory of attempted repair actions + outcomes so `DIAGNOSE` never repeats a failed action. Updates set `stale=True` rather than mutating `content`. |
| `Artifact`     | A typed output produced by a finished task (e.g. `DIRECTIONS_LIST`, `FINDINGS_BUNDLE`, `CODE_CHANGE_PLAN`, or the `DIAGNOSIS` verdict from **Phase 2**'s `DIAGNOSE` executor). Survives across turns. |
...
| `CLARIFY_REQUIREMENTS`  | Structured LLM call (or deterministic fallback) producing 3–6 clarification questions about the user's greenfield goal; emits a `REQUIREMENTS_SHEET`. **Schema-constrained (Phase 3.1)**: `CLARIFY_QUESTIONS_SCHEMA` (`cgx.answer.schemas`) rides as `json_schema` on the provider call so a schema-capable local backend decodes valid JSON by construction; the reply is checked by `validate_json_schema` with one bounded re-ask that folds the concrete violations back into the prompt before the deterministic fallback bank fires. **Scope calibration (P1.1)**: the executor also runs the deterministic `estimate_scope` (`cgx.session.scope`, no LLM) over the raw goal and stamps a `project_complexity` tier (`trivial`/`small`/`standard`/`complex`) + a `scope` block (`max_files`, `requested_features`) onto the `REQUIREMENTS_SHEET`; a `scope_estimate` trace record (`stage=clarify`) is emitted when tracing is on. *(greenfield mode)* |
| `DECOMPOSE`             | Folds the user's clarify answers into the goal, runs `plan_scaffold_manifest`, and emits a `WORK_PLAN` (`plan_md` + layered file manifest). **Programmatic Topological Order**: The file manifest is intercepted and hard-bucketed into 4 strict layers (Models -> Core -> API -> Tests) to enforce dependency resolution. **Schema-constrained (Phase 3.1)**: `MANIFEST_SCHEMA` rides as `json_schema` on the planning `provider.chat` call and the parsed reply gets one `validate_json_schema` re-ask (violations folded into the prompt) before the executor sees it. **Contract-first (P0)**: the plan now also carries a `contracts` block -- the *shared interfaces* every file must agree on, normalised by `_coerce_contracts` over four categories: `endpoints` (method + path, plus an optional success status code and message string -- P0c), `schemas` (model/class names), `functions` (name + owning module), `constants` (module-level names), and **a mandatory `project_skeleton`**. **Skeleton Generation Pass**: After planning, the pipeline calls `generate_project_skeleton` to write interface stubs for the entire tree and injects it into the contracts block. The block is threaded verbatim into every per-file generation so cross-file assumptions are declared once, not re-derived per file. **Mandatory cross-seam contracts (P0a)**: a manifest that pairs a JS/TS/Vue frontend caller (`.jsx`/`.tsx`/`.vue`) with a Python backend route (`_is_client_server_manifest` -- a canonical entry basename like `app.py`/`main.py`, a Flask/FastAPI/Django/Express skill or goal keyword, or a route signal in a file's description) talks over HTTP, so a request-key rename slips past every Python-only gate. When such a seam carries no `endpoints` contract, `_ensure_cross_seam_endpoints` runs one bounded, temperature-0 `extract_endpoint_contracts` pass (`cgx.answer.engine`) over the goal + file manifest to recover it; if that still yields nothing the executor fails **closed** with a non-`retryable` (terminal) error rather than scaffold the seam contract-free -- so P0b always has a contract to enforce. Pure-frontend or pure-backend manifests are unaffected. **Scope calibration (P1.1)**: before planning, `estimate_scope` (`cgx.session.scope`) recomputes the complexity tier over the *composed* goal (goal + clarify answers) and threads its "minimal viable stack" constraint into `plan_scaffold_manifest` as a hard `scope_constraint` (a file-count ceiling plus an explicit "do NOT introduce a DB / auth / frontend framework / browser-E2E tests unless the goal asks" directive) so a bare "calculator" can no longer pull in SQLAlchemy + migrations + Selenium + React; the resulting `project_complexity` + `scope` block is persisted on the `WORK_PLAN` and surfaced in the executor outputs (`project_complexity`, `scope_max_files`), and a `scope_estimate` trace record (`stage=decompose`) is emitted when tracing is on. **Plan self-critique (P1.2)**: once the manifest is coerced (and before the coherence gate), one bounded temperature-0 `critique_scaffold_manifest` (`cgx.answer.engine`) pass reviews the file list against that same scope ceiling and returns the paths it judges *speculative* (a DB layer / auth module / migrations / frontend / E2E suite the goal never asked for). The removal is advisory and **deterministic-safe** -- a missing provider or an unparseable reply yields no removals so the manifest is untouched (today's behaviour) -- and gated by `_safe_removals`, which refuses to drop a `depends_on` target, the last remaining source file (the whole critique is rejected wholesale rather than gut the tree), or the last test file; `_drop_files` then scrubs the survivors' dangling `depends_on` edges. The count is surfaced as `critique_removed` in the outputs and a `plan_critique` trace record (`stage=decompose`, `flagged`, `removed`, `removed_count`) is emitted when tracing is on. **Dependency/test de-scoping (P1.4)**: after the critique and before the coherence gate, `_descope_unrunnable` deterministically drops manifest files for a *sandbox-unrunnable* capability the goal never requested -- today only browser/E2E suites (Selenium/Playwright/Cypress, listed in `SANDBOX_UNRUNNABLE_FEATURES` in `cgx.session.scope`), which need a real display the headless agent environment does not provide. A file is flagged when its path or any string field (description/purpose) names one of the capability's `unrunnable_descope_needles`, and the same `_safe_removals` guardrails decide what is actually dropped (never a `depends_on` target, the last source file, or the last test file); an *explicitly requested* E2E suite (in `scope.requested_features`) is honoured and left intact, so the de-scope only ever trims speculative, unrunnable architecture. The count is surfaced as `descoped_files` in the outputs and a `plan_descope` trace record (`stage=decompose`, `unrunnable`, `removed`, `removed_count`) is emitted when tracing is on. **Coherence surgery as a plan-quality signal (P1.3)**: the deterministic coherence gate still repairs an incoherent manifest in place, but it now tallies its *heavyweight* structural repairs on a `CoherenceReport` -- relocated entry points (`_relocate_misplaced_stack_entries`), cut cross-language `depends_on` edges (`_repair_cross_language_deps`), and broken dependency cycles (`_find_dependency_cycle`); routine cheap repairs (dangling-edge prunes, injected entry files) are recorded for observability but never inflate the trigger. When the `surgery_score` reaches `COHERENCE_MUTATION_THRESHOLD` (3) *and* a `DECOMPOSE_RETRY_BUDGET` re-ask is still available, the executor returns a `retryable` failure whose message names the concrete defects (`_coherence_reask_constraint`), so `_decompose_retry_actions` re-plans once with that constraint folded into the goal rather than scaffolding a plan the gate had to heavily rewrite; on the retry (budget spent) it proceeds with the in-place-repaired manifest, so this is never worse than today's behaviour. The score is surfaced as `coherence_surgery` in the outputs and a `plan_coherence` trace record (`stage=decompose`, per-category counts + `surgery_score`) is emitted when tracing is on. *(greenfield mode)* |
...
* `on_task_completed(session, completed, tasks)` -- an executor
  ...
  - Repair loop (greenfield only): a fixable `VERIFY (assertions_failed |
    collection_error)` (or a `RUNTIME_VERIFY` boot failure) is funded via
    `LoopBudget.spend_repair` (the progress ledger keeps the loop alive
    while the failing-test count strictly drops round over round, under
    the absolute `REPAIR_BUDGET=4` ceiling, and the new
    `failure_signature` must not already be in
    `prior_failure_signatures`), then **forks on the gate's emitted
    `classification`** (**Phase 2**): a *reasoning-class* token
    (`assertion_drift` / `collection_error` / `unknown` /
    `runtime_failure`, the shared
    `cgx.session.repair.classify.DIAGNOSE_CLASSIFICATIONS` set) routes to
    `DIAGNOSE` instead of a mechanical `REPAIR`, while every mechanical
    token keeps its fast path straight to `REPAIR` -- the router gates by
    a pure membership test on the token the gate already stamped and never
    runs the classifier itself. `DIAGNOSE → {REPAIR | BOOTSTRAP_ENV |
    scoped SCAFFOLD | whole-tree regenerate}` is chosen by
    `_diagnose_dispatch_actions`, which maps the completed `DIAGNOSIS`'s
    `minimal_action` verdict to exactly one successor (`patch_files →
    REPAIR`, `add_dependency → BOOTSTRAP_ENV` install,
    `remove_dependency → BOOTSTRAP_ENV` descope,
    `adjust_manifest`/`regenerate_files →` scoped `SCAFFOLD` via
    `propose_regenerate(regenerate_files=…)`, `escalate →` today's
    whole-tree regenerate / `_replan_or_fail`) and always returns at least
    one action so the rung is never stranded; the same budget + flap guard
    ride the reasoning rung verbatim. The mechanical arms are unchanged:
  ...
  - Incremental re-verification (**Phase 3.2**, C2): when the
    `DIAGNOSE`-driven fix originated from a `VERIFY` failure, the fix
    builders stamp a `reverify_origin_gate` + `reverify_report_id` marker
    onto the spawned `REPAIR` / `BOOTSTRAP_ENV` node; the marker rides
    `inputs` down the fix chain and, at the edge that would normally
    re-enter verification (`_apply_to_verify` after a patch,
    `_bootstrap_to_api_check` after a dependency add / de-scope), the
    router splices a `RE_VERIFY` node (`_re_verify_node`) instead of the
    full `BOOTSTRAP_ENV → API_CHECK → SMOKE → VERIFY` chain -- the venv is
    already provisioned and every other gate already passed, so re-running
    them is wasted work. `RE_VERIFY` re-runs pytest against **only** the
    origin report's failing test file(s) and emits a shape-identical
    `VERIFY_REPORT`, so its successor (`_re_verify_successors`) delegates
    to `_verify_successors`: green hands off to `RUNTIME_VERIFY`, a
    still-failing reasoning-class outcome routes back to `DIAGNOSE` under
    the shared budget. Non-`VERIFY` origins (and the scoped-scaffold
    regenerate arm) carry no marker and run the full chain, so behavior is
...
* `on_task_failed(session, failed, tasks, retryable=False)` --
  ...
  `retryable` (an empty or unbuildable manifest, or one the coherence
  gate had to heavily repair -- **P1.3** -- a plan-quality problem, not
  a crash) is re-queued once by `_decompose_retry_actions` with the
  concrete failure folded into its goal as a constraint, bounded by
  `DECOMPOSE_RETRY_BUDGET=1`; a second identical failure is terminal. Explore-mode sessions keep
...
| `DECOMPOSE_RETRY_BUDGET` | 1 | Constraint-folded `DECOMPOSE` retries after a `retryable` executor failure (empty / unbuildable manifest, or heavy coherence surgery -- P1.3). |
...
1. `CLARIFY_REQUIREMENTS` runs on the raw objective. The executor
   ...
   `source: "llm" | "fallback"`, plus the **P1.1** `project_complexity`
   tier and `scope` block from `estimate_scope`).
...
3. `DECOMPOSE` runs once the answers are in. The executor folds
   each `Q: A` pair into the goal text, runs the deterministic
   `estimate_scope` (**P1.1**) over that composed goal to derive a
   "minimal viable stack" ceiling, calls
   `cgx.answer.engine.plan_scaffold_manifest` with that ceiling as a
   `scope_constraint`, then runs one bounded LLM self-critique
   (**P1.2**, `critique_scaffold_manifest`) that drops speculative
   files the goal never asked for -- guarded so it never removes a
   `depends_on` target or the last source/test file, and a no-op when
   the provider is unavailable. A deterministic de-scope pass then runs
   (**P1.4**, `_descope_unrunnable`) that drops any *sandbox-unrunnable*
   capability the goal never requested -- today browser/E2E suites that
   cannot run without a display -- under the same guardrails, so a plain
   "calculator" never ships a Selenium suite that could only fail. It
   emits a `WORK_PLAN` artifact
   carrying `plan_md` (human-readable markdown) plus
   `layers: [{name, files: [{path, description}]}]` and the
   `project_complexity` / `scope` block. An empty manifest is treated
   as a failure so the router can spawn a retry rather than push the
   session into a broken state. A manifest the deterministic coherence
   gate had to *heavily* repair (**P1.3**, `surgery_score ≥ 3` across
   relocated entries / cut cross-language edges / broken cycles) is
   likewise re-planned once via the same `DECOMPOSE_RETRY_BUDGET`, with
   the concrete defects folded into the goal so the planner fixes them at
   the source rather than shipping a heavily-rewritten tree to SCAFFOLD.
...
```mermaid
flowchart LR
    U([user goal]) --> CQ(["CLARIFY_REQUIREMENTS<br/>+ estimate_scope (P1.1)"])
    CQ --> DEC(["DECOMPOSE<br/>scope ceiling + self-critique + de-scope + coherence re-ask + contracts + layers"])
