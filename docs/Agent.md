...
3. `DECOMPOSE` runs once the answers are in. The executor folds
   ...
   `scope_constraint`, and emits a `WORK_PLAN` artifact carrying
   `plan_md` (human-readable markdown) plus
...
flowchart LR
    ...
    CQ --> DEC(["DECOMPOSE<br/>scope ceiling + contracts + layers"])
