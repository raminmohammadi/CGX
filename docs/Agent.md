...
3. `DECOMPOSE` runs once the answers are in. The executor folds
   ...
   the provider is unavailable. It emits a `WORK_PLAN` artifact
...
flowchart LR
    ...
    CQ --> DEC(["DECOMPOSE<br/>scope ceiling + self-critique + coherence re-ask + contracts + layers"])
